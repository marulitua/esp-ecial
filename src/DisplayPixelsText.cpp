...
void DisplayPixelsText::UpdateAnimation()
{
  ...
  int result = drawString(textStartX, 0, _text, textColor);
...
int  DisplayPixelsText::drawChar(uint8_t x, uint8_t y, uint8_t c, RgbColor color, uint8_t mode) {
  ...
  uint8_t i, j, temp;
  //Serial.printf("c = %d, fontStartChar = %d, fontTotalChar = %d", c, fontStartChar, fontTotalChar);
  ...
  if (rowsToDraw == 1) {
    for  (i = 0; i < fontWidth + 1; i++) {
      ...
      for (j = 0; j < 8; j++) { // 8 is the LCD's page height (see datasheet for explanation)
        if (temp & 0x1) {
          pixel(x + i, y + j + 2, color, mode);

        }
        else {
          pixel(x + i, y + j + 2, black, mode);
